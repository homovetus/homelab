...
static gboolean gst_example_meta_init(GstMeta *meta, gpointer params, GstBuffer *buffer) {
...
static gboolean gst_example_meta_transform(GstBuffer *dest, GstMeta *meta, GstBuffer *buffer, GQuark type, gpointer data) {
  GstExampleMeta *dmeta, *smeta;

  smeta = (GstExampleMeta *)meta;
  dmeta = (GstExampleMeta *)gst_buffer_add_meta(dest, GST_EXAMPLE_META_INFO, NULL);
...
const GstMetaInfo *gst_example_meta_get_info(void) {
  ...
  if (g_once_init_enter((GstMetaInfo **)&example_meta_info)) {
    const GstMetaInfo *meta = gst_meta_register(GST_EXAMPLE_META_API_TYPE, "GstExampleMeta",
                                                sizeof(GstExampleMeta), (GstMetaInitFunction)gst_example_meta_init,
                                                (GstMetaFreeFunction)NULL, gst_example_meta_transform);
    g_once_init_leave((GstMetaInfo **)&example_meta_info, (GstMetaInfo *)meta);
  }
...
GstExampleMeta *gst_buffer_add_example_meta(GstBuffer *buffer, gint x, gint y) {
  GstExampleMeta *meta;
  meta = (GstExampleMeta *)gst_buffer_add_meta(buffer, GST_EXAMPLE_META_INFO, NULL);
